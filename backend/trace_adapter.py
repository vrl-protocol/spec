...
class DeterministicTracePacket:
    ...
    def summary(self) -> dict[str, Any]:
        return {
            'input_hash': self.response.proof.input_hash,
            'output_hash': self.response.proof.output_hash,
            'trace_hash': self.response.proof.trace_hash,
            'integrity_hash': self.response.proof.integrity_hash,
...
def build_trace_packet(request_payload: object, *, produced_by: str = 'backend_integration', cycle: int = 1) -> DeterministicTracePacket:
    ...
    if response.proof.trace_hash != trace_artifact.trace_hash:
        raise RuntimeError('Trace artifact hash diverged from deterministic engine proof trace hash')
    if response.proof.input_hash != witness_artifact.input_hash or response.proof.output_hash != witness_artifact.output_hash:
